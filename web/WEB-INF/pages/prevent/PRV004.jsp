...
<spring:url value="/transaction/prevent/download-prevent-release-file" var="downloadPreventReleaseUrl"/>
<spring:url value="/transaction/prevent-print/${dataPreventInfor.prevent_id}" var="printViewUrl"/>
<style type="text/css">
    ...
    <div class="panel-group" id="accordion">
        ...
        <div class="padding-left-right">
            <div class="form-group">
                ...
                <label class="col-md-9 control-label  label-bam-trai">
                    <span style="color: red;" class="control-label">${dataPreventInfor.prevent_regist_agency}</span></label>
                            ...
                            <div class="col-md-4">
                                <c:if test="${dataPreventInfor.prevent_file_name.equals('') == false}">
                                    <c:forEach items="${listFileName}" var="item" varStatus="count">
                                        File ${count.index + 1} :
                                        <%--<a href="<%=SystemProperties.getProperty("url_config_server_stp_api")%>/prevent/download-prevent-from-stp/${dataPreventInfor.prevent_id}/${count.index}">${item}</a>--%>
                                        <a href="${downloadPreventUrl}/${dataPreventInfor.prevent_id}/${count.index}">${item}</a>
                                        <br>
                                    </c:forEach>
                                </c:if>
                                <c:if test="${dataPreventInfor.prevent_file_name.equals('') == true}">
                    ...
                    <c:if test="${thisProperty.release_flg == 1}">
                        <fieldset class="scheduler-border" style="border: 1px groove #ddd3 !important">
                            ...
                            <div id="collapseThree${loop.index}" class="panel-collapse collapse in">
                                ...
                                <div class="padding-left-right">
                                    <div class="form-group">
                                        <label class="col-md-3 control-label  label-bam-trai">Cơ quan đăng ký thông tin giải tỏa</label>
                                                ...
                                                <c:if test="${thisProperty.release_file_name!=null && thisProperty.release_file_name.equals('') == false}">
                                                    <c:forEach items="${thisProperty.arrReleaseFileName}" var="item" varStatus="count">
                                                        File ${count.index + 1} : <a
                                                            href="${downloadPreventReleaseUrl}/${thisProperty.id}/${count.index}">${item}</a><br>
                                                    </c:forEach>
                                                </c:if>
