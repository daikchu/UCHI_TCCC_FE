...
<spring:url value="/attestation-template/add" var="addUrl"/>
<spring:url value="/attestation-template/delete/${item.id}" var="deleteUrl"/>
<div id="menu-map">
    <a href="#menu-toggle" id="menu-toggle"><img src="<%=request.getContextPath()%>/static/image/menu-icon.png"></a>
    <span id="web-map">Thêm mới thông tin mẫu lời chứng</span>
</div>


<div class="truong-form-chinhbtt" ng-app="osp" ng-controller="attestationTempAddController">

    <div class="panel-group" id="accordion">
        <form id="formEdit" class="form-horizontal" action="${addUrl}" method="post">
            <input id="parent_code" type="hidden" name="parent_code">
            ...
            <div class="panel panel-default" id="panel1">
                ...
                <div class="panel-body">
                    <div class="form-group">
                        <label class="col-md-2 control-label label-bam-trai required">Lời chứng</label>
                        <div class="col-md-4">
                            <select name="template" ng-model="template.code"
                                    class="selectpicker select2 col-md-12 no-padding"
                                    ng-change="changeTemplate(template.code)">
                                <option value="">--Chọn loại lời chứng--</option>
                                <option ng-repeat="item in listAttestationTempParent" value="{{item.code}}">
                                    {{item.name}}
                                </option>
                            </select>
                            <div class="error_tooltip">{{invalid.type}}</div>
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-5 control-label label-bam-trai">
                            ...
                            <input class="truong-check-fix" type="radio" name="active_flg"
                                   value="0" ${item.active_flg==0?"checked":""} > Ngừng hoạt động
                        </div>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <textarea hidden class="col-md-12" id="giatriKindHtml" name="kind_html" rows="4"
                                  value="${item.kind_html}"></textarea>
                    </div>
...
<%--angularjs--%>
<script>
    myApp.controller('attestationTempAddController', ['$scope', '$http', '$window', function ($scope, $http, $window) {
        ...
        $scope.template = {
            ...
            $http.get(url + "/api-attestation/search", {params: {parent_code: '000'}})
            ...
            $http.get(url + "/api-attestation/search", {params: {code: code}})
                .then(function (response) {
                    ...
                    if(templates!=null && templates.length>0){
                        ...
                        $scope.template.kind_html = template.kind_html;
        ...
        $scope.BeforeSubmit = function () {
            ...
            $('#parent_code').val($scope.template.code);
            $('#formEdit').submit();
        };
        ...
        function checkValid() {
            ...
            if (isNullOrEmpty($scope.template.code) || $scope.template.code=='000') {
                $scope.invalid.type = "Trường không được để trống!";
                check = false;
            }
            return check;
        }
