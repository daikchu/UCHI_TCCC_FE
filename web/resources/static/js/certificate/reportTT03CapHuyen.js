myApp.controller('reportTT03CertCapHuyenController', ['$scope', '$http', '$filter', '$window', '$timeout', '$q', 'fileUpload', function ($scope, $http, $filter, $window, $timeout, $q, fileUpload) {
    ...
    $scope.loadCountDataCapHuyen = function () {
        $http.get(url + "/reportCertificate/tt03-list-xa-thuoc-huyen",
            {
                params: {
                    districtCode: $scope.districtCode,
                    dateFrom: $scope.searchCondition.dateFrom,
            ...
            .then(function (response) {
                $scope.reportTT03CapHuyenCerts = response.data;
