...
myApp.controller('contractAddController', ['$scope', '$http', '$filter', '$window', '$timeout', '$q', 'fileUpload', function ($scope, $http, $filter, $window, $timeout, $q, fileUpload) {
    ...
    $http.post(url+"/ContractKeyMap/getContractKeyMapByFilter", " where id = " + id, {headers: {'Content-Type': 'application/json'}})
        .then(function (response) {
            $scope.contract = response.data[0];
            if($scope.contract.type==4){
                $scope.check_type_special_word = true;
            }
            if($scope.contract.begin_or_close == 1 || $scope.contract.begin_or_close == 2){
                $scope.begin_or_close = true;
            }
            if(typeof $scope.contract.type != "undefined" && $scope.contract.type != null && $scope.contract.type != ""){
                $scope.contract.type = $scope.contract.type.toString();
                $scope.myFunc($scope.contract.type);
            }
            if(typeof $scope.contract.type_property != "undefined" && $scope.contract.type_property != null && $scope.contract.type_property != ""){
                $scope.contract.type_property = $scope.contract.type_property.toString();
            }
            ...
            $(document).ready(function() {
                $timeout(function() {
                    if($scope.contract.begin_or_close==1){
                        document.getElementById("begin_or_close_1").click();
                    } else if($scope.contract.begin_or_close==2) {
                        document.getElementById("begin_or_close_2").click();
                    } else if($scope.contract.begin_or_close==0) {
                        document.getElementById("begin_or_close_0").click();
                    }
                }, 0);
            });
    ...
    $scope.editKeyMap = function () {
        if($scope.validate()){
            ...
            $http.post(url+"/ContractKeyMap/check-exists-key", contractKeyMap, {headers: {'Content-Type': 'application/json'}})
                .then(function (response) {
                    if(response.data){
                        $http.post(url+"/ContractKeyMap/editContractKeyMap", contractKeyMap, {headers: {'Content-Type': 'application/json'}})
                            .then(function (response) {
                                if(response.data){
                                    $window.location.href = contextPath+'/contract/list-key-map-contract?status=2';
                                } else {
                                    $("#error").modal("show");
                                }
                            });
    ...
    $scope.begin_or_close_ = function (begin_or_close) {
        $scope.contract_copy = $scope.contract;
        if(begin_or_close == "0"){
            $scope.begin_or_close = false;
        ...
        if($scope.contract.type == "1"){
            if($scope.contract.type_property == "1"){  //cá nhân
                $scope.keyName = [
                    ...
                    {name:"Cùng nơi cư trú", map_var:"address_1"}
                ];
            ...
            if($scope.contract.type_property == "1"){  //Tài sản đất
                $scope.keyName = [
                    {name:"Quyền sử dụng đất của bên A đối với thửa đất theo giấy chứng nhận số", map_var:"land.land_certificate"},
                ...
                $scope.keyName = [
                    {name:"Căn hộ thuộc quyền sở hữu của bên A theo giấy chứng nhận số", map_var:"land.land_certificate"},
                    {name:"Nơi cấp", map_var:"land.land_issue_place"},
                    {name:"Ngày cấp", map_var:"land.land_issue_date"},
            ...
            $scope.keyName = [
                ...
                {name:"Tên công chứng viên", map_var:"notary_userFullName"}
            ];
        }
    };
