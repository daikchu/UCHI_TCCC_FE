...
// });
myApp.controller('contractOffAddCoppyController', ['$scope', '$http', '$filter', '$sce', '$window', '$q', '$timeout', 'fileUpload', function ($scope, $http, $filter, $sce, $window, $q, $timeout, fileUpload) {
        ...
        if (code > 0) {
            $http.get(url + "/contract/get-contract-template-by-code-template", {params: {code_temp: code}})
                .then(function (response) {
                    ...
                    if ($scope.contract.kindhtml != null && typeof $scope.contract.kindhtml != 'undefined' && $scope.contract.kindhtml != '' && $scope.contract.kindhtml.length > 0) {
...
//hungnn goi y khi nhap offline

    $scope.getsuggestoff = function (nprivy, npersons) {
    ...
    $scope.getsuggestpropertyoff = function (index, type) {
                    ...
                    $http.get(url + "/contract/contractNumber", {params: {year: year, userId: org_type == 1 ? userEntryId:0}})
                        .then(function (response) {
                            if(response.data != null){
                                ...
                                $scope.contract.contract_number = "1/" + year;
                            }
    ...
    $scope.downloadFile = function () {
        window.open(url + "/contract/download/" + id, '_blank');
    };
    ...
    $scope.temporaryOfflineAdd = function (typeOnline) {
        if ($scope.checkAllRequiredFieldTemporary()) {

            $scope.convertTemporary(typeOnline);
    ...
    $scope.checkAllRequiredFieldTemporary = function () {
        ...
        if ($scope.contractKind.contract_kind_code == null || $scope.contractKind.contract_kind_code == 'undefined' || $scope.contractKindValue == '') {
    ...
    $scope.convertTemporary = function (typeOnline) {
        if (typeOnline == 'true') {
            $scope.contractOfflineTemporary.kindhtml = $("div #contentKindHtml").html();
            $scope.contractOfflineTemporary.type = "3";
        } else {
        ...
        if ($scope.myFile.file != null && $scope.myFile.file != 'undefined' && $scope.myFile.file.size > 0) {
            if ($scope.myFile.file.size > 5242000) {
                ...
                fileUpload.uploadFileToUrl(file, uploadUrl)
                    .then(function (response) {
                            if (response.data != null && response.data != 'undefined' && response.status == 200) {
                                $scope.temporary.file_name = response.data.name;
                                $scope.temporary.file_path = response.data.path;

                                var contractAdd = JSON.parse(JSON.stringify($scope.temporary));
                                //contractAdd=contractAdd.replace("id","tcid");
