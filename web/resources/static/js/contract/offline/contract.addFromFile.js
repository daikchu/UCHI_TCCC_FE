...
myApp.controller('contractAddFromFileController', ['$scope', '$http', '$filter', '$window', 'fileUpload', '$timeout', '$q', function ($scope, $http, $filter, $window, fileUpload, $timeout, $q) {
    ...
    $scope.addContractWrapper = function () {
        if (org_type == 1) {//nếu là phường xã => gán id người công chứng = id tài khoản đăng nhập
            $scope.contract.notary_id = $scope.userEntryId;
        }
    ...
    $scope.importFileWord = function () {
        ...
        if ($scope.myFileImport.size > 5242000) {
            ...
            var file = $scope.myFileImport;
            var uploadUrl = url + "/contract/read-and-dissect-file";
            fileUpload.uploadFileToUrl(file, uploadUrl)
                .then(function (response) {
                        if (response.data != null && response.data != 'undefined' && response.status == 200) {
                            //refresh data
                            ...
                            $scope.contract.entry_user_id = entry_user_id_;
                            $scope.contract.entry_user_name = entry_user_name_;
                            $scope.contract.update_user_id = update_user_id_;
                            $scope.contract.update_user_name = update_user_name_;
                            $scope.contract.error_status = 0;
                            $scope.contract.mortage_cancel_flag = 0;
                            $scope.contract.cancel_status = 0;
                            $scope.contract.addition_status = 0;
                            ...
                            if ($scope.contract.json_property != null && $scope.contract.json_property != "" && $scope.contract.json_property != 'undefined') {
                                try {
                                    ...
                                    if($scope.listProperty.properties.length>0){
                                        var properties = $scope.listProperty.properties;
                                        for(var i = 0;i<properties.length;i++){
                                            $scope.changeTypeProperty(i,properties[i].type);
                                            if(properties[i].type=='01'){
                                                $scope.changePropertyRealEstate(i,'');
                                            }
                                        }
                                    }
...
//hungnn goi y khi nhap
    ...
    $scope.getsuggestproperty = function (index, type) {
        var namesuggest = "#land_certificate" + index;
