...
myApp.controller('temporaryDetailController', ['$scope', '$http', '$filter', '$window', '$sce', '$timeout', '$q', 'fileUpload', function ($scope, $http, $filter, $window, $sce, $timeout, $q, fileUpload) {
    ...
    $scope.changePropertyRealEstate = function(index, parent_code){
        ...
        $http.get(url + "/contract/get-list-property-real-estate-type", {params: {parent_code: parent_code}})
            .then(function (response) {
                ...
                if($scope.listProperty.properties[index].type_real_estate_sub != null && $scope.listProperty.properties[index].type_real_estate_sub.length>0){
    ...
    if (sub_template_id != null && sub_template_id != "" && sub_template_id > 0) {
        $http.get(url + "/contract/get-contract-template-by-code-template", {params: {code_temp: sub_template_id}})
            .then(function (response) {
                $scope.contractTemplatesApiece = [response.data];
                $scope.contractTemplates = $scope.contractTemplatesApiece;
                console.log($scope.contractTemplatesApiece);
            });
    }
...
//hungnn
    ...
    // phần lưu vào chỉnh sửa nếu không có thông tin công chứng viên
    ...
    $scope.downloadFile = function () {
        window.open(url + "/contract/temporary/download/" + id, '_blank');
    };
    ...
    $scope.addContractStore = function () {
        var validateAddContractStore = true;
        if (!$scope.contractKind.contract_kind_code > 0) {
            validateAddContractStore = false;
        }
    ...
    $scope.checkValidate = function () {
        ...
        if (typeof $scope.contractKind.contract_kind_code == "undefined" || $scope.contractKind.contract_kind_code == "" || !$scope.contractKind.contract_kind_code.length > 0) {
            check = false;
        }
        if (typeof $scope.contract.contract_template_id == "undefined" || $scope.contract.contract_template_id == "" || !$scope.contract.contract_template_id > 0) {
            check = false;
        }
        ...
        if ($scope.contract.contract_value < 0 || $scope.contract.contract_value == null || $scope.contract.contract_value == '') {
