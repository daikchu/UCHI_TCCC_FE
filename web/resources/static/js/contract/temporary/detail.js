...
myApp.controller('temporaryDetailController', ['$scope', '$http', '$filter', '$window', '$sce', '$timeout', function ($scope, $http, $filter, $window, $sce, $timeout) {
    $scope.url = url;
    $scope.contract = {};
    $scope.privys = {};
    $scope.listProperty = {};
...
//load default
            ...
            $http.put(url + "/contract/temporary", contractAdd, {headers: {'Content-Type': 'application/json'}})
                .then(function (response) {
                        if (response.status == 200 && response.data > 0) {
                            ...
                            var statusLoad;
                            $window.location.href = contextPath + "/contract/temporary/detailMark/" + id + "?statusLoad=1";
                            $scope.checkLoad = false;
                    ...
                    if(response.status == 200  && response.data>0){
                        ...
                        for(var i=0 ; i<$scope.privys.privy.length;i++){
                            ...
                            for( var j = 0;j<item2.persons.length;j++){
                                ...
                                var listPersonForm = {template:item4.template, id:item4.id, name:item4.name,birthday:item4.birthday,passport:item4.passport,certification_date:item4.certification_date,
                                    certification_place:item4.certification_place, address:item4.address, position:item4.position, description:item4.description,
                                    org_name:item4.org_name, org_address:item4.org_address, org_code:item4.org_code, first_registed_date:item4.first_registed_date, number_change_registed:item4.number_change_registed,
                                    change_registed_date:item4.change_registed_date, department_issue:item4.department_issue};
                        ...
                        $http.get(contextPath+"/contract/addPrivyFuncOnline",{params:{addData:listPersonAllJson,statusOnline:1}})
                            .then(function (response) {
                        ...
                        for(var i=0; i<$scope.listProperty.properties.length;i++){
                            ...
                            if((item3.type=="01" && item3.land.land_certificate != "") || (item3.type =="02" && item3.vehicle.car_frame_number !="") ){
                                var listPropertyForm  = {id:"", type:item3.type, type_view:item3.template,owner_info:item3.owner_info,other_info:item3.other_info,restrict:item3.restrict,
                                    land_certificate:item3.land.land_certificate, land_issue_place:item3.land.land_issue_place, land_issue_date:item3.land.land_issue_date, land_map_number:item3.land.land_map_number,
                                    land_number:item3.land.land_number, land_address:item3.land.land_address, land_area:item3.land.land_area, land_area_text:item3.land.land_area_text, land_public_area:item3.land.land_public_area,
                                    land_private_area:item3.land.land_private_area, land_use_purpose:item3.land.land_use_purpose, land_use_period: item3.land.land_use_period, land_use_origin:item3.land.land_use_origin, land_associate_property:item3.land.land_associate_property,
                                    land_street: item3.land.land_street, land_district: item3.land.land_district, land_province: item3.land.land_province, land_full_of_area: item3.land.land_full_of_area,land_type:item3.land.land_type,construction_area:item3.land.construction_area,
                                    building_area:item3.land.building_area,land_use_type:item3.land.land_use_type,land_sort:item3.land.land_sort, car_license_number: item3.vehicle.car_license_number, car_regist_number: item3.vehicle.car_regist_number,
                                    car_issue_place : item3.vehicle.car_issue_place, car_issue_date : item3.vehicle.car_issue_date, car_frame_number:item3.vehicle.car_frame_number, car_machine_number: item3.vehicle.car_machine_number, apartment_address: item3.apartment.apartment_address,
                                    apartment_number : item3.apartment.apartment_number, apartment_floor: item3.apartment.apartment_floor, apartment_area_use: item3.apartment.apartment_area_use, apartment_area_build:item3.apartment.apartment_area_build, apartment_structure: item3.apartment.apartment_structure,
                        ...
                        $http.get(contextPath+"/contract/addPropertyFuncOnline",{params:{addData:listPropertyAllJson,statusOnline:1}})
                            .then(function (response) {
                                console.log(response.status);
                                return response;
                            });
    ...
    $scope.downloadWord = function () {
        ...
        //sonnv lấy thư viện mới jquery.googoose.js
        // var o = {
        //     download: 0,
        //     filename: 'Hop dong cong chung.doc'
        // };
        // $(document).googoose(o);
        //end
    };
